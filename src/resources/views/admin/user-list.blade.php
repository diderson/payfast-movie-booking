		...
		<table class="table table-striped table-sm datatable" id="list_table">
			...
			<tbody>
				...
				<tr id="list_row_{{$data->id}}">
					...
					<td class="action">
						<a class="btn btn-primary btn-sm" href="{{route('users.edit', $data->id)}}">
							<span class="sr-only">Modify</span><i class="fa fa-pencil"></i>
						</a>

						<a class="btn btn-danger btn-sm item-remove" href="#" data-token="{{ csrf_token() }}" data-link="/admin/users/{{$data->id}}">
